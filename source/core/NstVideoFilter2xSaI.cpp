...
namespace Nes
{
	namespace Core
	{
		namespace Video
		{
			...
			{
				/*skin hack begin*/
				#define input_palette(p) p & 0x8000 ? ((p & 0x001F) << 3)+((p & 0x03E0) << 6)+((p & 0x7C00) << 9) : input.palette[p]
				// note that input.palette[] is replaced by the above define everywhere below
				/*skin hack end*/
				...
				for (uint y=0; y < HEIGHT; ++y)
				{
					for (uint x=0; x < WIDTH; ++x, ++src, dst[0] += 2, dst[1] += 2)
					{
						if (y)
						{
							i = x > 0 ?       input_palette(src[ -WIDTH-1 ]) : 0;
							e =               input_palette(src[ -WIDTH   ]);
							f = x < WIDTH-1 ? input_palette(src[ -WIDTH+1 ]) : 0;
							j = x < WIDTH-2 ? input_palette(src[ -WIDTH+2 ]) : 0;
						}

						g = x > 0 ?       input_palette(src[ -1 ]) : 0;
						a =               input_palette(src[  0 ]);
						b = x < WIDTH-1 ? input_palette(src[  1 ]) : 0;
						k = x < WIDTH-2 ? input_palette(src[  2 ]) : 0;

						if (y < HEIGHT-1)
						{
							h = x > 0 ?       input_palette(src[ WIDTH-1 ]) : 0;
							c =               input_palette(src[ WIDTH   ]);
							d = x < WIDTH-1 ? input_palette(src[ WIDTH+1 ]) : 0;
							l = x < WIDTH-2 ? input_palette(src[ WIDTH+2 ]) : 0;

							if (y < HEIGHT-2)
							{
								m = x > 0 ?       input_palette(src[ WIDTH*2-1 ]) : 0;
								n =               input_palette(src[ WIDTH*2   ]);
								o = x < WIDTH-1 ? input_palette(src[ WIDTH*2+1 ]) : 0;
							}
							else
							{
								m = n = o = 0;
